...
export default function ScheduleAdd({ navigation, id, setUpdate }) {
  const [selectDate, setDate] = useState(null);
  const [giveandTake, setGiveandTake] = useState("give");
  ...
  function handleAddSchedule() {
    if (selectDate && eventTarget && eventType && giftType && content) {
      ...
      axios.post(`https://don-forget-server.com/schedule/${id}`, {
        date: selectDate,
        type: eventType,
        event_target: eventTarget,
        gift: [giftType, content],
    ...
    <View style={styles.container}>
      ...
      <ModalDatePicker style={dateStyles.container}
        renderDate={({ year, month, day, date }) => {
          if (!date) {
            return <Text style={[dateStyles.text, dateStyles.placeholderText]}>경조사 날짜 *</Text>
          } else {
      ...
      <TouchableOpacity activeOpacity={0.8} style={styles.button} onPress={handleAddSchedule}>
